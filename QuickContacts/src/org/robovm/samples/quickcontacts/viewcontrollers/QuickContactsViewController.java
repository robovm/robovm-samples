...
public class QuickContactsViewController extends UITableViewController implements ABPeoplePickerNavigationControllerDelegate,
	...
	/**
	 * Checks current access status of
	 * current device's address book
	 */
	private void checkAddressBookAccess() {
	    ...
	    case   Denied:
	    case  Restricted:
            UIAlertView alert = new UIAlertView("Privacy Warning",
	...
	public double getRowHeight(UITableView tableView, NSIndexPath indexPath) {
		return (NSIndexPathExtensions.getSection(indexPath) == TableRowSelected.UIEditUnknownContactRow.ordinal()) ? UI_EDIT_UNKNOWN_CONTACT_ROW_HEIGHT : getTableView().getRowHeight();	
	}
