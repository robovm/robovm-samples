...
public abstract class ObjCObject extends NativeObject {
    ...
    static class ObjectOwnershipHelper {
        ...
        private static final long swizzledRetain = Selector.register("swizzled_retain").getHandle();
        private static final long release = Selector.register("release").getHandle();
        private static final long swizzledRelease = Selector.register("swizzled_release").getHandle();
        ...
        static {
            try {
                retainMethod = ObjectOwnershipHelper.class.getDeclaredMethod("swizzled_retain", ObjCObject.class, Long.TYPE);
                releaseMethod = ObjectOwnershipHelper.class.getDeclaredMethod("swizzled_release", Long.TYPE, Long.TYPE);
            } catch (Throwable t) {
                throw new Error(t);
            }
        }
        ...
        public static void registerClass(ObjCClass objCClass) {
            registerCallbackMethod(objCClass.getHandle(), retain, swizzledRetain, retainMethod);
            registerCallbackMethod(objCClass.getHandle(), release, swizzledRelease, releaseMethod);
        }

        private static void registerCallbackMethod(long cls, long originalSelector, long swizzledSelector, Method swizzledMethod) {
            long originalMethod = ObjCRuntime.class_getInstanceMethod(cls, originalSelector);
            long typeEncoding = ObjCRuntime.method_getTypeEncoding(originalMethod);

            long originalImp = ObjCRuntime.method_setImplementation(originalMethod, VM.getCallbackMethodImpl(swizzledMethod));
            if (!ObjCRuntime.class_addMethod(cls, swizzledSelector, originalImp, typeEncoding)) {
                throw new Error(
                      "Failed to register callback method on the ObjectOwnershipHelper: class_addMethod(...) failed");
            }
                
//            if (!ObjCRuntime.class_addMethod(cls, originalSelector, VM.getCallbackMethodImpl(swizzledMethod), typeEncoding)) {
//                throw new Error(
//                        "Failed to register callback method on the ObjectOwnershipHelper: class_addMethod(...) failed");
//            }
//            long x = ObjCRuntime.class_replaceMethod(cls, swizzledSelector, ObjCRuntime.method_getImplementation(originalMethod),
//                    typeEncoding);
        }

        @Callback
        private static @Pointer long swizzled_retain(ObjCObject self, @Pointer long sel) {
            ...
            return ObjCRuntime.ptr_objc_msgSend(self.getHandle(), swizzledRetain);
        }

        @Callback
        private static void swizzled_release(@Pointer long self, @Pointer long sel) {
            if (ObjCRuntime.int_objc_msgSend(self, retainCount) == 1) {
                synchronized (CUSTOM_OBJECTS) {
                    CUSTOM_OBJECTS.remove(self);
                }
            }
            ObjCRuntime.void_objc_msgSend(self, swizzledRelease);
        }
