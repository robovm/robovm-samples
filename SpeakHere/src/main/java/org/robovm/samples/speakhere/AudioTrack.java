...
public class AudioTrack {
    ...
    private final Vector<byte[]> mData = new Vector<>();
...
// inBuffer.setMAudioData(vp);
    ...
    public void play () {
        ...
        Runnable r = new Runnable() {
            @Override
            public void run () {
                AudioStreamBasicDescription asbd = new AudioStreamBasicDescription(mSampleRate, mFormatID, mFormatFlags,
                    mBytesPerPacket, mFramesPerPacket, mBytesPerFrame, mChannelsPerFrame, mBitsPerChannel, 0);
...
// AudioQueueError aqe = AudioQueue.newOutput(asbd, fp, vp, CFRunLoop.getCurrent(), new CFString(CFRunLoopMode.Common.value()), 0,
// mQueuePtr);
 ...
 * 
 * mQueue.stop(true); AQPlayerState.drop(mStateID);
 * 
