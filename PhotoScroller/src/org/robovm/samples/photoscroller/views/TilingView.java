...
public class TilingView extends UIView {
    ...
    /** To handle the interaction between CATiledLayer and high resolution screens, we need to always keep the tiling view's
     * contentScaleFactor at 1.0. UIKit will try to set it back to 2.0 on retina displays, which is the right call in most cases,
     * but since we're backed by a CATiledLayer it will actually cause us to load the wrong sized tiles. */
    @Override
    ...
    public void draw (CGRect rect) {
        ...
        /*
         * get the scale from the context by getting the current transform matrix, then asking for its "a" component, which is one
         * of the two scale components. We could also ask for "d". This assumes (safely) that the view is being scaled equally in
         * both dimensions.
         */
        ...
        /*
         * Even at scales lower than 100%, we are drawing into a rect in the coordinate system of the full image. One tile at 50%
         * covers the width (in original image coordinates) of two tiles at 100%. So at 50% we need to stretch our tiles to double
         * the width and height; at 25% we need to stretch them to quadruple the width and height; and so on. (Note that this
         * means that we are drawing very blurry images as the scale gets low. At 12.5%, our lowest scale, we are stretching about
    ...
    private UIImage getTile (double scale, int row, int col) {
        String tileName = String.format("Image_Tiles/%s_%d_%d_%d", imageName, (int)(scale * 1000), col, row);
