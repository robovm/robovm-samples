...
import org.robovm.samples.lazytableimages.ui.RootViewController;
...
public class LazyTableImages extends UIApplicationDelegateAdapter {
    ...
    public boolean didFinishLaunching(UIApplication application, UIApplicationLaunchOptions launchOptions) {
        ...
        appListFeedConnection = new NSURLConnection(urlRequest, new NSURLConnectionDataDelegateAdapter() {
            // The following are delegate methods for NSURLConnection. Similar
            // to callback functions, this is how
            // the connection object, which is working in the background, can
            // asynchronously communicate back to
            // its delegate on the thread from which it was started - in this
            // case, the main thread.

            /**
             * Called when enough data has been read to construct an
            ...
            /**
             * Called with a single immutable NSData object to the delegate,
             * representing the next portion of the data loaded from the
             * connection.
             */
            @Override
            ...
            /**
             * Will be called at most once, if an error occurs during a resource
            ...
            /**
             * Called when all connection processing has completed successfully,
            ...
            public void didFinishLoading(NSURLConnection connection) {
                ...
                // create an ParseOperation (NSOperation subclass) to parse the
                // RSS feed data
                ...
                // Referencing parser from within its completionBlock would
                ...
                weakParser.get().setCompletionBlock(new Runnable() {
                    ...
                    public void run() {
                        if (weakParser.get().getAppRecordList() != null) {
                            // The completion block may execute on any thread.
                            // Because operations
                            // involving the UI are about to be performed, make
                            // sure they execute
                            // on the main thread.
                            DispatchQueue.getMainQueue().async(new Runnable() {
                                @Override
                                public void run() {
                                    // The root rootViewController is the only
                                    // child of the navigation
                                    // controller, which is the window's
                                    // rootViewController.
                                    RootViewController rootViewController = (RootViewController) ((UINavigationController) getWindow()
                                            .getRootViewController()).getTopViewController();
                                    rootViewController.setEntries(weakParser.get().getAppRecordList());

                                    // tell our table view to reload its data,
                                    // now that parsing has completed
                ...
                queue.addOperation(parser); // this will start the
                                            // "ParseOperation"

                // ownership of appListData has been transferred to the parse
                // operation
        ...
        // Test the validity of the connection object. The most likely reason
        // for the connection object
        // to be nulll is a malformed URL, which is a programmatic error easily
        // detected during development
        // If the URL is more dynamic, then you should implement a more flexible
        // validation technique, and
        // be able to both recover from errors and communicate problems to the
    ...
    /**
     * Reports any error with an alert which was received from connection or
     * loading failures.
     * 
     * @param error
     */
