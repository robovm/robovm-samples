...
public class ButtonsViewController extends UITableViewController{
    ...
    public UIButton getAttrTextButton() {
        if ( attrTextButton == null) {
            ...
            normalAttrString.addAttribute(UIKit.ForegroundColorAttributeName(),
                               UIColor.colorRed(),
            ...
            normalAttrString.addAttribute(new NSString("Green"),
                    UIColor.colorGreen(),
                    new NSRange(0, normalAttrString.getLength()));

            attrTextButton.setAttributedTitle(highlightedAttrString, UIControlState.Highlighted);
            attrTextButton.setTag(viewTag); // tag this view for later so we can remove it from recycled table cells
        }
