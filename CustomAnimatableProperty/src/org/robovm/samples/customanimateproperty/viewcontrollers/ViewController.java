...
package org.robovm.samples.customanimateproperty.viewcontrollers;
...
/**
 * The view controller creates a few blub views which host the custom layer subclass.
 */
public class ViewController extends UIViewController{
	...
	public void viewDidLoad() {
	    ...
	    this.getView().setBackgroundColor(UIColor.colorYellow());
	    
	    // Load the bulb image.
	    UIImage bulb = UIImage.createFromBundle("bulb.png");
	    
	    // Base the size of the bulb views on the screen size.
	    double screenWidth = UIScreen.getMainScreen().getBounds().size().width();
	    double screenHeight = UIScreen.getMainScreen().getBounds().size().height();
	    double bulbHeight = screenHeight / 2.1;
	    
	    // Maintain image proportions by basing width on the width-to-height ratio.
	    double widthHeightRatio = bulb.getSize().width() / bulb.getSize().height();
	    System.err.printf("widthHeightRatio: %f", widthHeightRatio );
	    double bulbWidth = ( bulbHeight * widthHeightRatio ) * 1.5; // times 1.5 to fatten up the bulb for added visual effect.

	    // Define our view hierarchy.
	    BulbView bulbview;
	    CGRect startingFrame = new CGRect( 0, 0, bulbWidth, bulbHeight );
	    bulbview = new BulbView(startingFrame);
	    bulbview.setColor(UIColor.colorRed());
	    bulbview.setCenter(new CGPoint( .25 * screenWidth, .20 * screenHeight ));
	    this.getView().addSubview(bulbview);
	    bulbview = new BulbView(startingFrame);
	    bulbview.setColor(UIColor.colorGreen());
	    bulbview.setCenter(new CGPoint( .5 * screenWidth, .5 * screenHeight ));
	    this.getView().addSubview(bulbview);
	    bulbview = new BulbView(startingFrame);
	    bulbview.setColor(UIColor.colorBlue());
