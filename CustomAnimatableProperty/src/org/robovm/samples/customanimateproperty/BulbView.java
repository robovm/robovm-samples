...
/**
* View that hosts the custom CALayer subclass. Since the view hosts the layer in this case, 
* it executes the animations when explicit animations are the enabled animation trigger. 
*/
public class BulbView extends UIView  {
	...
	public BulbView() {
		...
		this.image = UIImage.createFromBundle("bulb.png");
		this.setFrame(new CGRect(0, 0, this.image.getSize().width(), this.image.getSize().height()));
	...
	void generalInit() {
	    ...
	    layer.setDelegate(new CALayerDelegateAdapter() {
	    	...
	    	public void drawLayer(CALayer layer, CGContext ctx) {
				...
				UIGraphics.beginImageContextWithOptions(image.getSize(), true, 1.0f);
	    ...
	    this.setColor(UIColor.colorRed());
	...
	public static ObjCClass getLayerClass(){
		...
		return ObjCClass.getByType(BulbLayer.class);
	}
	...
	private void animateFrom(double from, double to) {
		...
		if (Constants.ANIMATION_TYPE_KEYFRAME) {
			...
			((CAKeyframeAnimation)theAnimation).setCalculationMode(CoreAnimation.AnimationDiscrete());
			NSMutableArray<NSNumber> animationValues = new NSMutableArray<>();
		    ...
		    theAnimation.setDuration(1.0);
		    theAnimation.setTimingFunction(CAMediaTimingFunction.create(CoreAnimation.MediaTimingFunctionEaseOut()));
